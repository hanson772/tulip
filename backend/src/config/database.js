...
function getDb() {
  if (!db) {
    ...
    `);
    db.exec('CREATE INDEX IF NOT EXISTS idx_opinions_topic_id ON opinions(topic_id)');
    ...
    try { db.exec('ALTER TABLE opinions ADD COLUMN selectable INTEGER DEFAULT 1'); } catch (_) {}
    try { db.exec('ALTER TABLE opinions ADD COLUMN support_count INTEGER DEFAULT 0'); } catch (_) {}

    // Migration: add status column to topics
    try { db.exec('ALTER TABLE topics ADD COLUMN status TEXT DEFAULT "draft"'); } catch (_) {}
    db.prepare("UPDATE topics SET status = 'published' WHERE status IS NULL").run();

    // Migration: add deadline column to topics
    try { db.exec('ALTER TABLE topics ADD COLUMN deadline TEXT'); } catch (_) {}
    ...
    `);
    db.exec('CREATE INDEX IF NOT EXISTS idx_topic_tags_topic_id ON topic_tags(topic_id)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_topic_tags_tag_id ON topic_tags(tag_id)');
    ...
    `);
    db.exec('CREATE INDEX IF NOT EXISTS idx_supports_topic_id ON supports(topic_id)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_supports_user_id ON supports(user_id)');
    ...
    `);
    db.exec('CREATE INDEX IF NOT EXISTS idx_comments_topic_id ON comments(topic_id)');
